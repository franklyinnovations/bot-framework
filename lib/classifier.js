...
function GenerateClassifier(topicsToLoad) {
    ...
    topicsToLoad.filter(function (directory) { return typeof directory === 'string'; }).filter(function (directory) { return !_.startsWith(directory, '.'); }).forEach(function (directory) { return fs.readdirSync(directory).filter(function (directory) { return !_.startsWith(directory, '.'); }).forEach(function (topic) {
...
function readInTopic(topic, directory) {
    ...
    var actions = [];
    fs.readdirSync(directory).filter(function (file) { return !_.startsWith(file, '.'); }).forEach(function (file) {
